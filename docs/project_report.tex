...
\begin{abstract}
This report presents the complete design, implementation, and evaluation of a Smart City Traffic Management System. The system uses multi-agent reinforcement learning (Q-Learning and SARSA) to train adaptive traffic signal controllers across a 4$\times$4 grid of intersections, and applies multi-criteria decision making (WSM and TOPSIS) to evaluate and compare control policies. Five traffic scenarios---Normal, Rush Hour, Incident, Event, and Bus Priority---are simulated using synthetic datasets calibrated to real Dubai traffic patterns. A React.js dashboard provides interactive visualization of training progress, live simulation playback, scenario comparisons, and MCDM trade-off analysis. This document serves as a single source of truth: it contains every detail required to understand, reproduce, and extend the system.
\end{abstract}
...
\textbf{Q-Learning} \cite{watkins1992} is an off-policy temporal-difference method that learns the value of the optimal action in each state, regardless of the action actually taken. This allows faster convergence but can overestimate action values.

\textbf{SARSA} (State-Action-Reward-State-Action) \cite{rummery1994} is an on-policy method that updates Q-values based on the action actually taken by the current policy. This tends to learn more conservative (safer) policies because it accounts for exploration during learning.
...
The state is discretized for tabular Q-learning:
\begin{equation}
    s = (q_{NS}^d, q_{EW}^d, \phi) \quad \text{where } q^d = \min\left(4, \left\lfloor q / 5 \right\rfloor\right)
\end{equation}

This yields $5 \times 5 \times 2 = 50$ discrete states per intersection.
...
\begin{equation}
    \text{Safety} = \max\left(0, 100 - 5 \cdot N_{\text{near-miss}} - 0.1 \cdot N_{\text{stops}}\right)
\end{equation}

\subsection{Reward Function}

The reward for intersection $i$ at step $t$:
\begin{equation}
    R_i^t = \underbrace{-0.1(q_{NS} + q_{EW})}_{\text{queue penalty}} + \underbrace{0.05 \cdot V_{\text{passed}}}_{\text{throughput}} \underbrace{- 0.5 \cdot E_{\text{cumul}}}_{\text{emissions}} \underbrace{- 2.0 \cdot M}_{\text{safety}} \underbrace{- 0.05 \cdot |q_{NS} - q_{EW}|}_{\text{balance}}
\end{equation}

This multi-objective reward naturally encourages agents to:
\begin{itemize}[nosep]
    \item Minimize queue lengths (reduce wait times)
    \item Maximize vehicle throughput
    \item Reduce idle emissions
    \item Avoid unnecessary phase changes (reduce near-misses)
    \item Balance queues across directions
\end{itemize}
...
For dashboard playback, a separate method generates step-by-step simulation frames with low exploration ($\varepsilon = 0.1$) to show near-trained behavior. Each frame contains the full grid state, per-intersection queues, phases, actions, and rewards.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Styling}: Tailwind CSS 3.x (utility-first)
...
\begin{itemize}[nosep]
    \item Key metric cards: best throughput, lowest wait time, lowest emissions, highest safety
    \item Scenario selector dropdown
    \item Agent comparison summary table
\end{itemize}

\subsubsection{Live Simulation}
\begin{itemize}[nosep]
    \item Animated 4$\times$4 grid visualization
    \item Each cell shows: intersection ID, signal phase (green/red), queue lengths (NS/EW bars)
    \item Play/pause/step controls with adjustable speed
    \item Real-time metric charts updating each step (queue, throughput, emissions)
    \item Agent type selector to compare behavior visually
\end{itemize}

\subsubsection{Training Curves}
\begin{itemize}[nosep]
    \item Line charts for reward, wait time, throughput, emissions, safety over episodes
    ...
    \item Epsilon decay overlay on reward chart
\end{itemize}

\subsubsection{Scenario Comparison}
\begin{itemize}[nosep]
    \item Grouped bar charts: metric per agent per scenario
    \item Heatmap: agent$\times$scenario matrix colored by relative performance
...
\begin{itemize}[nosep]
    ...
    \item Interactive weight sliders to explore custom weight combinations
\end{itemize}

\subsubsection{Data Explorer}
\begin{itemize}[nosep]
    \item Summary statistics from synthetic datasets
    \item 24-hour traffic pattern visualization
    \item Emergency response time comparison (with vs. without preemption)
    \item Congestion heatmaps by zone and hour
\end{itemize}
...
\begin{longtable}{p{5cm}p{7cm}}
    ...
    live\_simulation\_*.json & Array of frame objects: \texttt{\{step, grid: [...], metrics, actions, rewards\}} \\
    scenario\_comparison.json & \texttt{\{scenario: \{label, agents: \{type: \{label, metrics\}\}\}\}} \\
    intersection\_meta.json & Array of \texttt{\{id, row, col, base\_volume\}} \\
    \bottomrule
\end{longtable}
...
\begin{itemize}[nosep]
    \item \textbf{Q-Learning}: Expected to converge within 150--200 episodes with aggressive exploitation after $\varepsilon$ decays. Typically achieves the highest throughput but may have slightly lower safety scores due to frequent phase switching.
    \item \textbf{SARSA}: Slightly slower convergence (200--250 episodes) but often produces more stable, conservative policies. May have marginally higher safety scores.
    \item \textbf{Fixed-Timer}: Constant performance across all episodes (no learning). Serves as the lower bound.
\end{itemize}

\subsection{Scenario Impact}

\begin{itemize}[nosep]
    \item \textbf{Rush Hour}: All agents see increased wait times and queue lengths due to $2.5\times$ traffic volume. RL agents should show the greatest improvement over Fixed-Timer here.
    \item \textbf{Incident}: Rerouted traffic challenges RL agents to adapt to asymmetric load. Fixed-Timer cannot adapt, causing significant congestion.
    \item \textbf{Event}: Localized traffic spikes test whether RL agents can prioritize affected intersections.
    \item \textbf{Bus Priority}: RL agents with bus-priority preemption should show better public transport metrics with slight throughput reduction.
\end{itemize}

\subsection{MCDM Rankings}

The expected ranking (from WSM/TOPSIS with default weights) is typically:
\begin{enumerate}[nosep]
    \item Q-Learning or Q-Learning + Bus Priority (best efficiency-safety-emissions balance)
    \item SARSA or SARSA + Bus Priority (slightly more conservative)
    \item Fixed-Timer (worst overall, no adaptation)
\end{enumerate}

Sensitivity analysis may reveal that under heavy safety weighting, SARSA surpasses Q-Learning due to its conservative on-policy nature.
...
\begin{enumerate}[nosep]
    \item \textbf{Tabular RL over Deep RL}: The state space (50 states per intersection) is small enough for tabular methods, which are more interpretable and guaranteed to converge with sufficient exploration.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Fixed state discretization}: 50 states may lose information in high-traffic scenarios. Deep Q-Networks (DQN) with continuous states could improve scalability.
...
This project demonstrates that multi-agent reinforcement learning can significantly outperform fixed-timer traffic control across diverse traffic scenarios. Q-Learning and SARSA agents learn adaptive signal timing policies that reduce wait times, increase throughput, and lower emissions compared to the non-adaptive baseline. The MCDM framework (WSM and TOPSIS) provides a principled method for comparing policies across multiple competing objectives, with sensitivity analysis revealing the robustness of rankings to weight changes. The React.js dashboard makes these complex results accessible through interactive visualization, enabling stakeholders to explore trade-offs and make informed decisions about traffic management strategies.
